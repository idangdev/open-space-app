...
const fakeErrorReponse = new Error('Ups, something went wrong');
...
describe('asyncPopulateUsersAndTalks thunk', () => {
  ...
  it('should dispatch action and call alert correctly when data fetching failed', async () => {
    // arrange
    // stub implementtion
    api.getAllUsers = () => Promise.reject(fakeErrorReponse);
    api.getAllTalks = () => Promise.reject(fakeErrorReponse);
    // mock dispatch
    const dispatch = vi.fn();
    // mock alert
    window.alert = vi.fn();
    ...
    expect(dispatch).toHaveBeenCalledWith(showLoading());
    expect(dispatch).toHaveBeenCalledWith(hideLoading());
    expect(window.alert).toHaveBeenCalledWith(fakeErrorReponse.message);
  });
});
